...
class PrestaShopAutoload
{
    ...
    {
        // Retrocompatibility   别名存在对应并且加载的名称本身不存在，动态创建继承类
        if (isset(PrestaShopAutoload::$class_aliases[$classname]) && !interface_exists($classname, false) && !class_exists($classname, false)) {
            return PsKits::setAlias($classname,PrestaShopAutoload::$class_aliases[$classname]);
        }
            ...
            if ($file[0] != '.') {
                if (is_dir($root_dir.$path.$file)) {
                    ...
                    $pattern = '#\W((abstract\s+)?class|interface)\s+(?P<classname>'.basename($file, '.php').'(?:Core)?)'
                                .'(?:\s+extends\s+'.$namespacePattern.'[a-z][a-z0-9_]*)?(?:\s+implements\s+'.$namespacePattern.
                        '[a-z][\\a-z0-9_]*(?:\s*,\s*'.$namespacePattern.'[a-z][\\a-z0-9_]*)*)?\s*\{#i';
                    ...
                    if (preg_match($pattern, $content, $m)) {
                        ...
                        if (substr($classname, -4) == 'Core') {
                            $classes[substr($classname, 0, -4)] = array(
                                'path' => '',
